(*
  ...
  Abstract:
    ...
    for the necessity of the Trinity. Moving beyond traditional Anselmian "perfection,"
...
(*
  =============================================================================
  PROJECT   : Formal Verification of the Ontological Trinity Necessity
...
text \<open> [Definition: PH (Philosophical H)]
  ...
  Formal Equivalence: PH q <-> (EH q AND TH q) \<close>
...
consts Ref :: "bool \<Rightarrow> bool"     (* epistemic refutation; no axioms *)
text \<open>
  Definition:
...
exists)
so, EDia\_ep(p) <=> negation of  Refuted(p)
...
to ``EDia\_ep (@{text "\<Omega>"} @{text "\<and>"} @{text "\<Psi>"})"(ES) — no Val-Op, no global axioms\<close>
...
subsection \<open>14.5 Minimal assumptions @{text "\<Rightarrow>"} sandwich existence (no-1, no-2, no-4+, no-superfluous)\<close>
...
subsection \<open>14.8 Pull+NS package\<close>
text \<open>-- A. Adapters --\<close>
...
text \<open>-- D. One-shot NS discharge (EH-free) --\<close>
...
text \<open>
  ...
  (A) Model-backed confirmation (Sections 8,9 of @{file Diagnostics_Optional.thy}).
 Sections 8,9 of @{file Diagnostics_Optional.thy} provides a concrete satisfiable witness model (e.g., via Nitpick)
  ...
  Even if we do not appeal to the Sections 8,9 of @{file Diagnostics_Optional.thy} witness, the epistemic role of
...
(* ---------------------------------------------------------
   ...
   --------------------------------------------------------- *)
text \<open>
Sections 8,9 of @{file Diagnostics_Optional.thy} provide a concrete satisfiable witness
...
proof
  ...
  have imp_to_conj:
    ...
    using Edge_implies_notOneHead by blast
...
section \<open>20. Packaging (T)+(S): consequences we can still derive\<close>
...
section \<open>28. Final Locale Audit: Discharging Core and Uniqueness \<close>
text \<open>
